...
public final class Class_o {
	...
	public static void Init(final String language) {
		...
		Class_o.var_29f5 = getProfilesRegions();
		...
		if (unlocked != null && !unlocked.equals("1") && unlocked.equals("0")) {
			...
			if ((Class_o.igpCode = getAppProperty("IAP-GameCodeIGP")).length() == 0) {
				...
				new StringBuffer().append("PaySMS.parseJadFields:iapTestField: ").append(Class_o.iapTestField)
						.append(" useTestProfile: ").append(Class_o.useTestProfile);
	...
	public static boolean verifyRequest(int inputCode) {
		...
		if ((Class_o.var_2a45 = sub_6d7d()) != null && Class_o.var_2a45.length() > 0) {
			equals = String.valueOf(inputCode).equals(String.valueOf(Integer.parseInt(Class_o.var_2a45) ^ 0xD0A4));
		}
			...
			if (Class_o.var_29a5[i][2].indexOf(s) != -1) {
				for (int j = 0; j < Class_o.var_29b5[i].size(); ++j) {
					...
					if (str != null) {
						...
						new StringBuffer().append("PaySMS.getProfilesCarrierAndIds: regionName = ").append(s)
								.append(", carrierName = ").append(str);
						if ((Class_o.var_2a1d == null || sub_702c(str, Class_o.var_2a1d))
								&& !vector.contains(str.substring(0, n))) {
	...
	public static String sub_54b2() {
		...
		new StringBuffer().append("PaySMS.GetTermsAndConditions: tnc without phone number = '").append(sub_5bf6)
				.append("'");
		return sub_5bf6;
	}
	...
	private static int sub_59b0(final int n, final String s) {
		...
		if (s.equals("Cash")) {
			for (int i = 0; i < Class_o.cashVector.size(); ++i) {
				if (((Integer) Class_o.cashVector.elementAt(i)).intValue() == n) {
					n2 = i;
					break;
				}
			}
			...
			for (int j = 0; j < Class_o.coinVector.size(); ++j) {
				if (((Integer) Class_o.coinVector.elementAt(j)).intValue() == n) {
					n2 = j;
					break;
				}
			}
	...
	private static boolean sub_5a7c(final int n, final String s) {
		...
		new StringBuffer().append("PaySMS.isValidContentID: IAP-ContentID-").append(s).append("-").append(n)
				.append(": ").append(sub_5e3c).append(b ? " - Valid" : " - Invalid");
		return b;
	}
	...
	private static String sub_5bf6(final String s, final String str) {
		...
		try {
			...
			if (str == null || str.equals("")) {
				...
				s2 = substring
						+ (substring2.substring(0, index3) + str
	...
	public static int getPrice(final int pricepoint) {
		if (GetSpecialFlow() == 6) {
			if (Class_o.currentValidProfiles == null) {
				return 0;
			}
